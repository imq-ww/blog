...
public class TagController {
    ...
    @GetMapping("/tags")
    public String tagsPage(@PageableDefault(size = 4,sort = {"id"},direction = Sort.Direction.DESC)Pageable pageable,
                           Model model){
    ...
    @PostMapping("/tags")
    public String saveTags(@Valid Tag tag, BindingResult result, RedirectAttributes attributes){
    ...
    @PostMapping("/tags/{id}")
    public String editTag(@Valid Tag tag,BindingResult result,RedirectAttributes attributes,@PathVariable("id") Long id){
        Tag t = tagService.getTagByName(tag.getName());
        if (t!=null){
            result.rejectValue("name","nameError","不能添加重复标签");
        }
